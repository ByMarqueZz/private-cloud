...
app.post("/api/login", async (req, res) => {
  ...
  connection.query(
    "SELECT hash FROM users WHERE username = ? AND password = ?",
    [username, password],
    (err, rows, fields) => {
      if (err) throw err;
      if (rows.length > 0) {
...
app.post("/api/createFile", async (req, res) => {
  ...
  fs.writeFile("./" + path + "/" + fileName, "", (err) => {
    ...
    connection.query(
      "INSERT INTO files (name, path, user_id, type, permissions) VALUES (?, ?, ?, ?, ?)",
      [fileName, path, body.user_id, fileExtension, body.permission],
      (err, rows, fields) => {
        if (err) throw err;
        connection.query(
...
app.get("/api/getifFollow/:user_id/:user_logged_hash", async (req, res) => {
  ...
  connection.query(
    "SELECT * FROM follows WHERE following_id = ? AND follower_id = (SELECT id FROM users WHERE hash = ?)",
    [user_id, user_logged_hash],
    (err, rows, fields) => {
      if (err) throw err;
      res.json(rows);
    },
...
app.get("/api/follow/:user_id/:user_logged_hash", async (req, res) => {
  const { user_id, user_logged_hash } = req.params;
  connection.query(
    "INSERT INTO follows (following_id, follower_id) VALUES (?, (SELECT id FROM users WHERE hash = ?))",
    [user_id, user_logged_hash],
    (err, rows, fields) => {
      if (err) throw err;
      res.json(true);
...
app.get("/api/unfollow/:user_id/:user_logged_hash", async (req, res) => {
  const { user_id, user_logged_hash } = req.params;
  connection.query(
    "DELETE FROM follows WHERE following_id = ? AND follower_id = (SELECT id FROM users WHERE hash = ?)",
    [user_id, user_logged_hash],
    (err, rows, fields) => {
      if (err) throw err;
      res.json(true);
...
app.get("/api/getPath/:path?", (req, res) => {
  ...
  connection.query(
    `SELECT files.*, IFNULL(users.profile_picture, '') AS shared_profile_picture, IFNULL(users.username, '') AS shared_username
    FROM files
      ...
      connection.query(sql, params, (err, rows, fields) => {
        ...
        connection.query(
          ...
          (err, rows, fields) => {
            if (err) throw err;
            connection.query(
              "SELECT id, level FROM users WHERE hash = ?",
              [req.body.hash],
              (err, rows, fields) => {
                if (err) throw err;
                let idLevel = getFrameIdLevel(rows[0].level);
...
app.post("/api/sendFile", async (req, res) => {
  ...
  try {
    ...
    if (!fs.existsSync("./" + userSelected.username + "/Compartido")) {
      ...
      connection.query(
        "INSERT INTO files (name, user_id, path, permissions, type, shared_by_id) VALUES (?, ?, ?, ?, ?, ?)",
        [
          "Compartido",
          userSelected.id,
    ...
    if (
      fs.existsSync(
        "./" + userSelected.username + "/Compartido/" + body.file.name,
      )
    ) {
      newUniqueName = getUniqueFolderName(
        "./" + userSelected.username + "/Compartido/",
        body.file.name,
      );
    }
    if (body.file.type != "folder") {
      fs.copyFile(
        "./" + path + "/" + body.file.name,
        "./" + userSelected.username + "/Compartido/" + newUniqueName,
        (err) => {
          if (err) throw err;
...
function copyFolderToSend(
  ...
  fs.mkdirSync(
    "./" + userSelected.username + "/Compartido/" + path + newFolder,
  );
  let newPath = path;
  if (path != "") {
  ...
  connection.query(
    ...
    [
      newFolder,
      userSelected.id,
      userSelected.username + "/Compartido" + newPath,
      0,
      "folder",
      shared_user_id,
    ],
    (err, rows, fields) => {
      if (err) throw err;
      connection.query(
        "SELECT * FROM files WHERE path = ? AND password is NULL",
        [pathOrigin],
        (err, rows, fields) => {
          if (err) throw err;
          rows.forEach((file) => {
      ...
      for (let i = 0; i < file.length; i++) {
        ...
        file[i].mv("./" + path + "/" + fileName, (err) => {
          if (err) {
            ...
            connection.query(
              "INSERT INTO files (name, path, user_id, type, permissions) VALUES (?, ?, ?, ?, ?)",
              [fileName, path, req.body.user_id, fileExtension, permissions],
              (err, rows, fields) => {
                if (err) throw err;
                if (i != 0) {
                  addProgress(req.body.user_id, "upload", idLevel, (param) => {
                    console.log(param);
                  });
                }
              },
...
function getUniqueFileName(path, fileName) {
  ...
  while (fs.existsSync(`${path}/${name}`)) {
    ...
    const fileNameWithoutExtension = nameWithoutparentheses.replace(
      `.${extension}`,
      "",
    );
...
app.post("/api/createFolder", (req, res) => {
  ...
  connection.query(
    "INSERT INTO files (name, path, user_id, type, permissions, password) VALUES (?, ?, ?, ?, ?, ?)",
    [
      folderName,
      path,
      req.body.user_id,
...
function addProgress(user_id, hecho, idLevel, callback) {
  ...
  connection.query(
    "INSERT INTO progress(user_id, do) VALUES (?, ?)",
    [user_id, hecho],
    (err, rows, fields) => {
      if (err) throw err;
      connection.query(
        "SELECT * FROM missions WHERE callback = ?",
        [hecho],
        (err, rows, fields) => {
          if (err) throw err;
          if (rows.length > 0) {
            let mission = rows[0];
            connection.query(
              "SELECT count(*) as count FROM progress WHERE user_id = ? AND do = ?",
              [user_id, hecho],
              (err, rows, fields) => {
                if (err) throw err;
                ...
                if (count >= mission.max_value) {
                  connection.query(
                    "SELECT * FROM users_passed_missions WHERE user_id = ? AND mission_id = (SELECT id FROM missions WHERE callback = ?)",
                    [user_id, hecho],
                    (err, rows, fields) => {
                      if (err) throw err;
                      if (rows.length === 0) {
                        connection.query(
                          "INSERT INTO users_passed_missions(user_id, mission_id) VALUES (?, (SELECT id FROM missions WHERE callback = ?))",
                          [user_id, hecho],
                          (err, rows, fields) => {
                            if (err) throw err;
                          },
...
app.post("/api/delete", async (req, res) => {
  ...
  connection.query(
    "DELETE FROM files WHERE name = ? AND path = ? OR path = ?",
    [name, path, path + "/" + name],
    (err, rows, fields) => {
      if (err) throw err;
    },
...
app.get("/api/getIsPublic/:path/:file", (req, res) => {
  ...
  connection.query(
    "SELECT permissions FROM files WHERE path = ? AND name = ?",
...
app.post("/api/rename", async (req, res) => {
  ...
  try {
    fs.rename(fullPath, path.join(formData.path, formData.newName), (err) => {
      ...
      connection.query(
        "UPDATE files SET name = ?, permissions = ?, password = ? WHERE name = ? AND path = ? AND type = ?",
        [
          formData.newName,
          formData.permission,
          formData.password,
        ...
        (err, rows, fields) => {
          ...
          connection.query(
            ...
            [
              formData.path + "/" + formData.newName,
              formData.path + "/" + formData.lastName,
            ],
            (err, rows, fields) => {
              if (err) throw err;
...
app.get(
  "/api/solicitudRegistro/:email/:name/:surname/:password/:username",
  (req, res) => {
    const salt = bcrypt.genSaltSync(13);
    connection.query(
      "INSERT INTO users (email, name, surname, password, username, hash, profile_picture, level, points) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        req.params.email,
        req.params.name,
        req.params.surname,
...
app.get("/api/icons/:type", (req, res) => {
  ...
  connection.query(
    "SELECT path FROM icons WHERE type = ?",
    [type],
    (err, rows, fields) => {
      if (err) throw err;
      if (rows.length > 0) {
