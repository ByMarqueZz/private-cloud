    ...
    return (
      <div className="container">
        ...
        <form className='formLogin'>
          <div className='divInputLogin'>
            <div className="form-floating inputLogin">
              <input required type="text" className="form-control" id="floatingName" placeholder="Nombre de usuario" value={user} onChange={(event) => setUser(event.target.value)}/>
              <label htmlFor="floatingPassword">Nombre de usuario</label>
            </div>
          </div>

          {
            userIncorrect ? <p className="alert alert-danger" role="alert">El usuario introducido no forma parte de nuestra aplicación</p> : null
          }

          <button onClick={handleSubmit} type="submit" className='loginButton btn btn-primary'>Restablecer contraseña</button>
        </form>
      </div>
    );
        }
